...
class RubinParquetReader(ParquetReader):
    ...
    def read(self, input_file, read_columns=None):
        ...
        kwargs = {} if read_columns is None else {"columns": read_columns}
        table = pq.read_table(input_file, **kwargs).to_pandas()
        # for smaller_table in parquet_file.to_batches(max_chunksize=self.chunksize):
        #    table = pa.Table.from_batches([smaller_table]).to_pandas()
        #    yield self.process_rubin_table(table)
        # for table in parquet_file.to_pandas():
